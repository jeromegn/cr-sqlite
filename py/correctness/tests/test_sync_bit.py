...
def test_insert_row():
    ...
    c = create_db()
    c.execute(
        "INSERT INTO crsql_changes VALUES ('foo', x'010901', 'b', 1, 4, 4, x'1dc8d6bb7f8941088327d9439a7927a4', 3, 6, 1)")
    c.commit()
...
def test_update_row():
    ...
    c.commit()
    c.execute(
        "INSERT INTO crsql_changes VALUES ('foo', x'010901', 'b', 1, 4, 4, x'FFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFF', 3, 6, 1)")
    changes = c.execute("SELECT * FROM crsql_changes").fetchall()
...
def test_delete_row():
    ...
    c.execute("INSERT INTO foo VALUES (1, 2)")
    c.commit()
    c.execute("INSERT INTO crsql_changes VALUES ('foo', x'010901', '-1', 1, 4, 4, x'FFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFF', 4, 6, 1)")
    c.commit()
    changes = c.execute("SELECT * FROM crsql_changes").fetchall()
    assert (changes == [('foo',
                        b'\x01\t\x01',
...
def test_custom_trigger():
    ...
    c.commit()
    c.execute(
        "INSERT INTO crsql_changes VALUES ('foo', x'010902', 'b', 1, 4, 4, x'FFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFF', 3, 6, 1)")
    rows = c.execute("SELECT * FROM log").fetchall()
    assert (rows == [(1, 1), (2, 1)])
