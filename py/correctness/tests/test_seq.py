...
def test_increments_by_one_in_tx():
    ...
    rows = c.execute("SELECT seq FROM foo__crsql_clock ORDER BY seq ASC").fetchall()
    assert (rows == [(0,), (1,)])
...
def test_resets_on_every_tx():
    ...
    rows = c.execute("SELECT seq FROM foo__crsql_clock ORDER BY db_version ASC, seq ASC").fetchall()
    assert (rows == [(0,), (1,), (0,), (1,)])
...
def test_incr_by_one():
    ...
    rows = c.execute(
        "SELECT db_version, seq FROM bar__crsql_clock ORDER BY db_version ASC, seq ASC").fetchall()
    assert (rows == [(6, 0), (6, 1)])
    ...
    rows = c.execute("SELECT seq FROM baz__crsql_clock ORDER BY seq ASC").fetchall()
    assert (rows == [(0,), (1,)])
